...
7. Do NOT fabricate data the user doesn't have`}]},contents:[...l,{role:`user`,parts:[{text:e}]}],generationConfig:{maxOutputTokens:200,temperature:.7}}}function t(e){let t=e.split(`
`),n=[],r=[];for(let e of t)if(e.startsWith(`ACTION:`))try{let t=JSON.parse(e.substring(7));n.push(t)}catch{}else r.push(e);let i=r.join(`
`).trim();return{text:i,actions:n,html:i.replace(/\*\*(.+?)\*\*/g,`<strong>$1</strong>`).replace(/\*(.+?)\*/g,`<em>$1</em>`).replace(/`(.+?)`/g,`<code class="px-1.5 py-0.5 bg-white/10 rounded text-accent">$1</code>`).replace(/^- (.+)$/gm,`<li>$1</li>`).replace(/\n/g,`<br>`)}}function n(e,t){return{system_instruction:{role:`system`,parts:[{text:`You are a focus coach summarizing a user's browsing history for a specific category (${t}).
